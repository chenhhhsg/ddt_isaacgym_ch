...
class D1F12DOF(LeggedRobot):
    ...
    def post_physics_step(self):
        ...
        # vec_f = self.feet_pos[:, 0, :] - self.feet_pos[:, 1, :]  # FL - FR
        # vec_r = self.feet_pos[:, 2, :] - self.feet_pos[:, 3, :]  # RL - RR
        # vec_l_f = quat_rotate_inverse(self.base_quat, vec_f)
        # vec_r_r = quat_rotate_inverse(self.base_quat, vec_r)

        # foot_x_err_f = torch.abs(vec_l_f[:,0]).mean()
        # foot_x_err_r = torch.abs(vec_r_r[:,0]).mean()
        # print(f"body-frame span x: left={foot_x_err_f.item():.3f}m right={foot_x_err_r.item():.3f}m")
