...
class D1HFlat(LeggedRobot):
    ...
    def _reward_feet_air_time(self):
        ...
        rew_airTime *= torch.abs(self.commands[:, 1]) > 0.1 
...
class D1HFlatCfg( LeggedRobotCfg ):
...
class D1HFlatCfg_Play( D1HFlatCfg ):
    class env(D1HFlatCfg.env):
        num_envs = 10
    class init_state( D1HFlatCfg.init_state ):
        pos = [0.0, 0.0, 0.5] # x,y,z [m]
    class terrain(D1HFlatCfg.terrain):
        mesh_type = 'plane'  # "heightfield" # none, plane, heightfield or trimesh
        num_rows = 5
        num_cols = 5
        ...
        curriculum = False
    class noise( D1HFlatCfg.noise ):
        add_noise = False
    class control ( D1HFlatCfg.control ):
        use_filter = True
    class domain_rand( D1HFlatCfg.domain_rand ):
        push_robots = False
...
class D1HFlatCfgPPO( LeggedRobotCfgPPO ):
